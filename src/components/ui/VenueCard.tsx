...
export const VenueCard: React.FC<VenueCardProps> = ({ venue }) => {
  ...
  const getCategoryName = (v: Venue) => {
    ...
    if (v.name.includes('Resort')) return 'Wedding Resort';
    if (v.name.includes('Kalyana Mandapam')) return 'Kalyana Mandapam';
    if (v.name.includes('Destination')) return 'Destination Wedding Venue';
    if (v.name.includes('Farmhouse')) return 'Wedding Farmhouse';
    if (v.type === 'Heritage') return 'Heritage Palace & Mandap';
    if (v.type === 'Banquet') return 'Grand Banquet Hall';
    if (v.type === 'Resort') return 'Luxury Wedding Resort';
    if (v.type === 'Temple') return 'Sacred Kalyana Mandapam';
    if (v.type === 'Ashram') return 'Sacred Heritage Retreat';
    return `${v.type} Venue`;
  };
